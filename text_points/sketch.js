...
function setup() {
	...
	let ps = font.textToPoints(displayText,width/2-textWidth(displayText)/2,height/2+tSize/2,tSize)
	for(let p of ps) {
		points.push(new Point(createVector(random(width),random(height)),createVector(p.x,p.y)))
	}
...
function setWord() {
	...
	for(let pi in ps) {
		...
		if(pi < points.length){
			...
			if(points.length) {
				points.push(new Point(random(points).pos.copy(),createVector(p.x,p.y)))
			}else{
				points.push(new Point(p5.Vector.random2D().setMag(middle.mag()),createVector(p.x,p.y)))
			}
		}
	}
...
class Point {
	...
	addForce(vec) {
		...
		let v = p5.Vector.sub(this.pos,vec).mult(1/pow(d,distancePower)*force)
		this.vel.add(v)
	}
